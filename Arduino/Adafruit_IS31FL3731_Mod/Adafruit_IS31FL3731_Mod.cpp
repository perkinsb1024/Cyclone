...
void Adafruit_IS31FL3731::all_on(uint16_t color) {
  // all LEDs on & 1 PWM
  ...
  for (uint8_t i=0; i<6; i++) {
    ...
    // write 24 bytes at once
    for (uint8_t j=0; j<24; j++) {
      Wire.write((byte) color);
    }
    Wire.endTransmission();
  }
}

void Adafruit_IS31FL3731::drawPixelBuffer(uint8_t *color, uint8_t start, uint8_t length) {
  // Only drawing 24 at a time because that's what the original library does...
  // For some reason, it's only drawing 16 total though...
  uint8_t max = 24;
  selectBank(_frame);
  while(length > 0) {
    if(length < max) {
...
void Adafruit_IS31FL3731::disableLedPage(uint8_t page) {
  writeRegister8(_frame, page, 0);
}

void Adafruit_IS31FL3731::enableLedPage(uint8_t page) {
  writeRegister8(_frame, page, 255);
}
